...
func (k *AsyncKafkaProducer) Send(value []byte) {
	...
	case <-k.isClose:
		...
		// 直接返回，此条消息浪费掉了，如果后期需要收集未发送成功的消息可以在此收集
		// 输出到日志或者等待producer重启成功后再发送
		return
	default:
		// 正常情况发送消息
		...
		for {
			...
			case rc := <-producerError:
				if rc != nil {
					...
					k.isClose <- struct{}{}
					common.Logger.Error(sendMessageError, rc)
				}
				return
			// 发送成功
			case res := <-producerMessage:
				encode, _ := res.Value.Encode()
				common.Logger.Debug(string(encode))
			}
		}
	}(producer)
